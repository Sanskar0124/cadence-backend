...
const getLeaderboardData = async (req, res) => {
  try {
    ...
    let result = [];
    // * fetch admin
    const [admin] = await UserRepository.findUserByQuery({
      user_id: req.user.user_id,
    });

    // * fetch admin's company's all users
    const [users, errForUsers] =
      await DepartmentRepository.getAllEmployeesWithoutAdmin(admin.company_id);
...
const getLeaderboardGraph = async (req, res) => {
  try {
    ...
    /*
    	// Fetch user with user_id and admin.company_id (to ensure admin can view this user)
		- If no user found, reject.
	*/

    let [user, userErr] = await UserRepository.findUserByQuery({ user_id });
    if (userErr || user.company_id !== admin.company_id)
