...
const myformat = combine(
  ...
  align(),
  printf((info) => {
    let user_id = info.user_id ? ` 👤 ${info.user_id} ~ ` : '';
    ...
    //TODO: Refactor codebase to remove the following
    if (info.stack) {
      try {
        let stackTrace = info.stack
          ...
          .split(')')[0]
          .trim();
        return `${info.timestamp} ${info.level}:${user_id}${info.message} \t Traceback: CRM-BACKEND${stackTrace}`.trim();
      } catch (err) {
